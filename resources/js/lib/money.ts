...
/**
 * A file size, in the locale's digits.
 *
 * Unit suffixes stay Latin: KB and MB are used as-is in Arabic and Kurdish
 * technical writing, and inventing localised ones would be worse than leaving
 * them.
 */
export function formatFileSize(bytes: number, locale: string): string {
    if (bytes < 1024) {
        return `${formatDecimal(bytes, locale, { maximumFractionDigits: 0 })} B`;
    }

    if (bytes < 1024 * 1024) {
        return `${formatDecimal(bytes / 1024, locale, { maximumFractionDigits: 1 })} KB`;
    }

    return `${formatDecimal(bytes / (1024 * 1024), locale, { maximumFractionDigits: 1 })} MB`;
}
